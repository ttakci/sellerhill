...
- `/admin` is a lazy, role-gated read-only frontend with Queues / Costs / Users tabs. Only `UserRole.ADMIN` sees the sidebar item; backend guards remain authoritative. In-page warnings use `ADMIN_QUEUE_WAITING_THRESHOLD` (default 100), `ADMIN_KEEPA_LOW_TOKENS_THRESHOLD` (default 100), and `ADMIN_LLM_FAILURE_RATE_THRESHOLD` percent (default 10). External alert delivery and destructive queue actions remain intentionally absent.
