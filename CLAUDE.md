...
**Landing page** (`apps/web/src/features/landing/`) — public marketing page at `/` (no locale prefix). **Light, clean SaaS design (rebuilt 2026-08-12)**, TR/EN + light/dark aware.
...
- **The hero follows the theme; it is no longer unconditionally dark.** An earlier revision of this file said the opposite and it was load-bearing, so read this carefully: `colors.landing.heroText`/`heroTextMuted` are now **real ink** (dark in light theme, light in dark theme) and must NOT be used on a brand-coloured fill. Text/icons sitting on `brand.primary` use **`colors.landing.onAccent`** (always light, both themes). The animated aurora panel behind the auth pages (`MeshBackground`) draws white particles and must stay dark, so it has its own **`colors.landing.auroraBg`** — it used to borrow `heroBg`, which was only safe while the hero was dark. Never merge those two back.
...
- **The locale control is the app's, not a second dialect of it.** The navbar uses a `Dropdown` with a `TR ⌄` trigger matching `AppShell.style.ts`'s `LanguageSelectTrigger`, not the `LanguageSwitcher` molecule's segmented `EN|TR` pair — a visitor who signs up should meet the same control they just used.
...
- **`<Logo>` on the landing uses `layout="nav"`**, like the app shell. The default layout is the tall mark-plus-tagline drawn for the 280px auth panels; at navbar height it collapses into an illegible smudge.
- Icons: the `<Icon>` component resolves `color="brand.primary"` / `"semantic.success"` / `"landing.<token>"` dot-paths. Brand icons are `amazon`/`ebay` in the icon map; `brand-amazon`/`brand-ebay` are aliases. Marketing icons available: `arrow-right`, `star`, `zap`/`sparkles`, `shield-check`, `clock`, `layers`, `repeat`, `gauge`, `rocket`, `storefront`, `chart-line`, `bolt`, `eye`.
...
**Standing prerequisites (both are enforced in code, not conventions):**
1. **A fixed ISP proxy must be resolvable for the user.** `assertCanEnable` refuses to turn the flag on without one (`amazon.errors.autoFulfillProxyRequired`) and `runForOrder` re-checks at runtime, blocking with `proxy_required` rather than running bare-IP. A proxy was in place for the 2026-07-30 dry-run — checkout could not otherwise have reached the review step. **Production pool sizing is still open**: the model is one static IP per USER (~$3-4/proxy/30d, `/admin` → Proxies, migrations `057`/`058`), so 500 users means ~500 proxies — the platform's largest recurring cost by a wide margin.
2. **A real Amazon buyer account** (email + password + 2FA secret) with `auto_fulfill_enabled=true` and `auto_fulfill_cap_total` set. TOTP is answered from the stored secret — fulfillment must never require a human to type a code.
