...
- **Both write paths build their bodies from `ebay-listing-payload.ts`.** A bulk-published listing must be byte-for-byte what the single path would publish — the rules encoded there (GS1 check digits, the BrandMPN pair, tag-safe truncation, the image floor) have each broken live listings once. `listing-invariants.guard.spec.ts` greps that neither path forks.
...
- **There is no bulk kill switch and no per-item create path to fall back to.** A per-item fallback would cost 25× the quota for byte-identical output, so the toggle's only possible effect was to make things worse. `useBulk = Boolean(request.ebayAccountId)`; the per-ASIN worker survives only for a request naming no store, which the UI cannot produce.
- **Drafts run through the same batch and stop before the writes.** A draft costs **zero** eBay calls — category and item specifics are resolved by `publishListing` later — so draft→publish is 3 calls total, exactly what a direct create costs. A draft defers the quota, it never doubles it, and an abandoned draft costs nothing. The AI title generated at draft time is preserved at publish (`applyContentAi: false` there, and the stored title wins), so the LLM also runs once.
...
- `ListingProcessorService` now honours `failure.details.retryable`; it previously rethrew regardless, so a permanently broken ASIN re-paid Keepa, the LLM rewrite and the whole publish sequence three times (~27 eBay calls) to be refused again.
...
**`listings.sku` + `listings.ebay_offer_id` (migration `067`)** are the precondition. The offer id was previously thrown away at create and re-fetched on every update (one of the four calls); the SKU was re-derived as `${asin}-NEW`, which is silently wrong in sandbox where create mints a timestamped SKU. Production SKUs are backfilled; sandbox rows stay NULL and self-heal via the surviving lookup.
...
| `067` | `listings.sku` + `listings.ebay_offer_id` — the two identifiers `bulkUpdatePriceQuantity` addresses rows by; removes the per-update offer lookup. Production SKUs backfilled, sandbox left NULL (timestamped, not derivable) |
