...
**Two provider groups (2026-07-31).** `LlmService.resolveProvider` picks base URL + API key + model by purpose group, so listing traffic and the customer chatbot can be billed and scaled separately:
- **Listing group** (`CONTENT`, `ASPECT`, ingestion) → `LLM_PROVIDER` / `LLM_BASE_URL` / `LLM_API_KEY` / `LLM_CONTENT_MODEL`. Runs across thousands of products per bulk add, so it wants the **cheapest tier that does the job** — the task is "compress a title to 80 chars" and "pick one value from an allowed list", not deep reasoning.
- **Assistant group** (`ASSISTANT*`) → `LLM_ASSISTANT_BASE_URL` / `LLM_ASSISTANT_API_KEY` / `LLM_ASSISTANT_MODEL`. Each value **falls back to the listing group when unset**, so a single-provider deployment is unchanged. Never hardcode a non-empty default for `LLM_ASSISTANT_MODEL` in a compose file — a model id from another provider sent to `LLM_BASE_URL` is a 404.
...
**Compose files.** `docker-compose.yml` = local dev services (Postgres/Redis/pgAdmin — no LLM service; see "Provider/config"). `docker-compose.test.yml` = Coolify test/sandbox (eBay sandbox endpoints, debug logging, concurrency 1). `docker-compose.production.yml` = Coolify production (eBay production endpoints). The two Coolify files build the same images from the same code and differ only in configuration; both mount `browser_state` + `fulfillment_evidence` volumes, define a `/api/health/live` healthcheck, and carry a memory limit sized for resident Chromium contexts.
