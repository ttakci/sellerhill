...
**One panel, no duplicates.** `/:locale/admin` is the single admin surface, with tabs Overview / Queues / Costs / Proxies / Settings / Billing / Users. The former `/:locale/admin/assistant` page duplicated the overview + queue + usage reads and was removed; the path now redirects to `/admin`. Do not add a second admin page — add a tab.
...
- The admin frontend renders only redacted data — no prompts, messages, credentials, or raw provider errors are exposed. Support console is available at `/:locale/support` for SUPPORT/ADMIN users. Provider invoice reconciliation and external alert delivery remain follow-ups.
...
- `/admin` is a lazy, role-gated frontend with Overview / Queues / Costs / Proxies / Settings / Billing / Users tabs. Only `UserRole.ADMIN` sees the sidebar item; backend guards remain authoritative. Container logic is split into `features/admin/hooks/useAdminProxyForm.ts` and `useAdminSettings.ts`. Money is rendered with `formatMicroCurrency` from `@repo/ui` (micro-USD → localized currency); an unknown (null) cost renders as an em dash, never `0`. Destructive queue actions remain intentionally absent.
