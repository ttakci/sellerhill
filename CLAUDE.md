...
- PREDEFINED templates resolve through `ListingSettingsGroupService.getPredefinedTemplateHtml(id)` (5-min in-process cache); a missing id degrades to `DEFAULT_LISTING_TEMPLATE_HTML`, never to an empty description. Seeding **upserts by name** — the old delete-and-reinsert minted new ids and orphaned every group's `predefinedTemplateId`. It also self-heals rows that seed already duplicated: extra same-name rows are deleted, and groups pointing at them are repointed to the surviving id **before** the delete (otherwise the id dangles and the create path silently drops to the default template).
...
- `createListingWithRest` **throws `ListingPublishExhaustedError`** when the retry loop ends without a listing id. It used to `return { listingId: '' }` as success, and both write sites recorded an ACTIVE listing with an empty `ebay_item_id` — a listing that does not exist on eBay, can never be matched to an order (order sync keys on that column), and blocks re-listing the ASIN. Both write sites now also guard on `!ebayItemId`.
...
**Two provider groups, split by economics (2026-07-31).** `LlmService.resolveProvider` picks base URL + API key + model by purpose group, so listing traffic and the customer chatbot never share a bill:
- **Listing group** (`CONTENT`, `ASPECT`, ingestion) → `LLM_BASE_URL` / `LLM_API_KEY` / `LLM_CONTENT_MODEL`. This runs across thousands of products per bulk add, so it belongs on a **local model** (Ollama) and must stay free.
- **Assistant group** (`ASSISTANT*`) → `LLM_ASSISTANT_BASE_URL` / `LLM_ASSISTANT_API_KEY` / `LLM_ASSISTANT_MODEL`. Each value **falls back to the listing group when unset**, so a single-provider deployment is unchanged and the chatbot costs nothing until a paid provider is actually configured. Point these at OpenAI/Groq the day the chatbot goes live — bulk listing traffic stays local.

**`LlmUsagePurpose.ASPECT`** is the third use: choosing a required eBay item specific from the category's own allowed-value list (see "Listing quality" above). It runs on the listing group, is gated by `ebay.aspects.llmEnabled` (default off, admin-togglable) and capped by `ebay.aspects.llmMaxPerListing` (default 3). Its answer is validated against the allowed list and **written back as a learned default**, so each `(category, aspect)` question is asked once and every later listing in that category costs zero tokens. `llm_usage_log.purpose` separates the spend from content and assistant.
...
- Dev/trickle default: local Ollama (`LLM_BASE_URL=http://localhost:11434/v1`, `llama3.2:3b`). **Use an instruct model, never a reasoning one** — verified live 2026-07-31: `qwen3:1.7b` spends its entire token budget inside a `<think>` block and Ollama's OpenAI-compatible endpoint returns an EMPTY `content` (`finish_reason: length`), even with `/no_think`, so every AI title/description silently fell back to the deterministic one. `ContentGenerationService.stripReasoning` additionally removes inline `<think>` blocks for models that keep them in `content`.
- **Model choice was measured, not assumed** (CPU, 2026-07-31, same prompt + two real products): `llama3.2:3b` used 61-73 of the 80 characters and kept the model number / pack size every time; `qwen2.5:3b-instruct` (1.6s) and `gemma2:2b` (2.1s) came back at 46-53 characters and dropped identifiers like `1400B` and `16 Pouches`. Warm latency: title ~1.6-2.4s, description ~11s (65 tokens at ~8 tok/s). Since the description dominates the cost and the title dominates eBay search ranking, **title-only AI is the sensible default** for bulk sellers. `docker-compose.yml` includes the service + persistent `ollama_data` volume; pull once with `docker compose exec ollama ollama pull qwen3:1.7b` (and `qwen3:4b-instruct` for the future assistant).
...
**An account is either a seller account or a staff account, never both.** Zonds ships two products behind one login: the seller app (dashboard / listings / orders / settings) and the operator console (`/admin`, `/support`). They used to share one shell, so an ADMIN was a seller account with extra menu items. That is gone.

| Role | May reach |
|---|---|
| `CUSTOMER` | Seller app only. No support console, no admin panel. |
| `SUPPORT` | Support console only — answers customer questions; not staff of the platform's configuration. |
| `ADMIN` | Admin panel + support console (queue **read-only**; see below). |
...
- **The operator surfaces are exactly five**: `admin.controller`, `knowledge-admin.controller`, `support.controller`, `auth.controller` and `profile.controller` (the last two are session/self-service, needed by every account). `account-scope.guard.spec.ts` unit-tests the guard **and** source-greps that list, so adding a sixth is a deliberate edit — marking a seller controller would hand staff a customer's data back.
- **ADMIN in the support console is read-only.** `SupportController` accepts `@Roles(UserRole.SUPPORT, UserRole.ADMIN)`, but `SupportService.authority` still restricts every acting route (claim / reply / transfer / resolve / presence) to SUPPORT — the queue, capacity and transfer targets are modelled on support agents (`support_profiles`), so an admin oversees the queue without becoming an agent in it. Only `list` and `detail` pass `SUPPORT_CONSOLE_READ_ROLES`.
- **Web enforcement**: `apps/web/src/layouts/OperatorLayout/` is the staff shell for `/:locale/admin` and `/:locale/support` — its own sidebar (Admin for ADMIN, Support for both), no seller nav, no store switcher, no `AssistantWidget`. `AppLayout` bounces staff to the console; `OperatorLayout` bounces sellers to `/dashboard`. Both shells share one set of styled components (`layouts/shell/AppShell.style.ts`) and re-export it, so the chrome can never fork.
- **`apps/web/src/app/operatorRouting.ts` is the single routing authority** for staff: the console route table, `resolveHomePath(role, hasConnectedAccounts)` (used by login, register, Google auth and both layout guards, so they cannot disagree) and `resolveOperatorBreadcrumbs`. `routeMeta.ts` describes the seller app only and deliberately no longer lists `/admin` or `/support`.
...
The admin module lives in `apps/api/src/modules/admin/` and uses the existing assistant/auth RBAC (`UserRole.CUSTOMER|SUPPORT|ADMIN`, `Roles`, `RolesGuard`, `PrivilegedSessionGuard`, migration `041`). It does not introduce a second role system. Every admin route is gated by `JwtAuthGuard` + `RolesGuard` + `PrivilegedSessionGuard` (per-request session revalidation) + `@Roles(UserRole.ADMIN)`.
...
**One panel, no duplicates.** `/:locale/admin` is the single admin surface — rendered inside the **operator shell**, not the seller shell (see "Account scope" above) — with tabs Overview / Queues / Costs / Proxies / Settings / Billing / Users. The former `/:locale/admin/assistant` page duplicated the overview + queue + usage reads and was removed; the path now redirects to `/admin`. Do not add a second admin page — add a tab.
...
- The admin frontend renders only redacted data — no prompts, messages, credentials, or raw provider errors are exposed. Support console is available at `/:locale/support` for SUPPORT (full) and ADMIN (read-only) — both inside the operator shell, never the seller shell (see "Account scope"). Provider invoice reconciliation and external alert delivery remain follow-ups.
...
**Compose files.** `docker-compose.yml` = local dev services (Postgres/Redis/pgAdmin/Ollama). `docker-compose.test.yml` = Coolify test/sandbox (eBay sandbox endpoints, debug logging, concurrency 1). `docker-compose.production.yml` = Coolify production (eBay production endpoints). The two Coolify files build the same images from the same code and differ only in configuration; both mount `browser_state` + `fulfillment_evidence` volumes, define a `/api/health/live` healthcheck, and carry a memory limit sized for resident Chromium contexts.
...
- **Body is 15px, not 16.** `typographyTokens.fontSize.base` (0.9375rem) is the primary reading size and the value-text size for every form control (TextInput/Select/SearchField/MessageComposer). `fontSize.md` (16px) is now the **heading** step (`h4`), not body. Heading leading is tight (`lineHeight.display` 1.1 / `tight` 1.2 / `snug` 1.3) because Lexend's x-height already carries the row; body is `normal` 1.47 (15/22).
...
| `body` | 15px / regular | Primary UI text, control text, row labels |
