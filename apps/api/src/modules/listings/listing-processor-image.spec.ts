...
describe('attachMirroredImage', () => {
  ...
  it('attaches our URL when the mirror succeeds', async () => {
    ...
    await attachMirroredImage(service as never, 'p1', product, null);
  ...
  it('leaves the field undefined when the mirror fails', async () => {
    const product = { imageUrls: ['https://a/images/I/71nx65qZq6L.jpg'] } as never;
    await attachMirroredImage(mirror(null) as never, 'p1', product, null);
  ...
  it('never throws, whatever the mirror does', async () => {
    ...
    await expect(attachMirroredImage(service as never, 'p1', product, null)).resolves.toBeUndefined();
  });

  /**
   * Threading test for the Finding-1 fix: the stored name passed into
   * `attachMirroredImage` must reach `ensureMirrored` completely unchanged —
   * never coerced to a boolean, never re-derived from the product's own
   * `imageUrls[0]`. `ensureMirrored`'s reuse path depends on receiving the
   * exact stored name, not a truthiness signal.
   */
  it('forwards the stored mirrored image name to ensureMirrored unchanged', async () => {
    const product = { imageUrls: ['https://a/images/I/ROTATED-CURRENT-NAME.jpg'] } as never;
    const service = mirror('https://img.example.com/71nx65qZq6L.jpg');
    await attachMirroredImage(service as never, 'p1', product, '71nx65qZq6L.jpg');
    expect(service.ensureMirrored).toHaveBeenCalledWith(
      'p1',
      'https://a/images/I/ROTATED-CURRENT-NAME.jpg',
      '71nx65qZq6L.jpg'
    );
  });
});
