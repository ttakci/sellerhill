 ...
 * `Invalid Base32 string: Unknown letter: " "`. That error surfaces only much
 * later — at scrape/checkout time inside `performLogin`, where
 * `AmazonCheckoutService.ensureLoggedIn` misclassifies it as a `login` block —
 * so the account looks credential-broken when only the paste format was wrong.
 *
 * Normalizing at BOTH boundaries (write: create/update; read: getDecrypted)
 * means a pasted secret works, and secrets already stored with spaces keep
 * working without the user re-entering them.
 */
