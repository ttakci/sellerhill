-- Re-skins the transactional email templates seeded in 014_seed_email_templates.sql
-- to match the app's actual brand tokens (brand.primary #2563eb, the landing hero
-- gradient, semantic.warning, the accent/emerald checklist color) instead of the
-- generic purple gradient + rocket-emoji header the templates shipped with.
-- An applied migration never re-runs, so this UPDATEs the existing rows rather
-- than editing 014 in place.
--
-- The header carries the actual SellerHill mark (the rounded badge + ascending
-- bars from packages/ui/src/atoms/Logo, inverted to a white badge with blue
-- bars for contrast on the gradient header) instead of a bare wordmark, with an
-- MSO conditional-comment fallback (a plain "S" tile) for Outlook desktop,
-- which does not render inline SVG. CTA buttons use the bulletproof
-- table+td[bgcolor]+a pattern with the text color inlined on the anchor itself
-- (`!important` too) — a <style> class alone is not reliable across email
-- clients, several of which strip <head><style> or override anchor color with
-- their own default link blue, which is what was making the button text
-- unreadable.
