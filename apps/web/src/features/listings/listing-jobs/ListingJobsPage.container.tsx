...
export const ListingJobsPageContainer: React.FC = () => {
  ...
  // Dynamic polling: Poll as long as there are pending/processing jobs
  const { data: jobs = [], isLoading, refetch } = useGetListingJobsQuery(undefined, {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (status: string) => (
        <S.StatusBadge $status={status}>
          {t(`listings.jobs.status.${status.toLowerCase()}`)}
        </S.StatusBadge>
      )
    },
    {
      ...
      render: (_: any, job: any) => {
        const percent = job.totalAsins > 0 
    ...
    },
    {
      ...
      render: (_: any, job: any) => (
        <S.StatsContainer>
          <S.SuccessText>{job.successCount} {t('listings.jobs.stats.success')}</S.SuccessText>
          <S.FailedText>{job.failedCount} {t('listings.jobs.stats.failed')}</S.FailedText>
    ...
    },
    {
      ...
      render: (date: string) => <S.DateText>{new Date(date).toLocaleString(t('translation:common.languageCode') || 'en-US')}</S.DateText>
    },
    {
      key: 'actions',
      ...
      render: (_: any, job: any) => (
        <S.ActionButton onClick={() => handleViewDetails(job.id)}>
