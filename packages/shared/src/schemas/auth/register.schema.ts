    ...
    .object({
      firstName: z
        ...
        .min(
          ...
          t('validation.minLength', { min: AUTH_CONSTANTS.FIRST_NAME_MIN_LENGTH })
        )
        .max(
          AUTH_CONSTANTS.FIRST_NAME_MAX_LENGTH,
          t('validation.maxLength', { max: AUTH_CONSTANTS.FIRST_NAME_MAX_LENGTH })
        ),
      lastName: z
        .string()
        .min(
          AUTH_CONSTANTS.LAST_NAME_MIN_LENGTH,
          t('validation.minLength', { min: AUTH_CONSTANTS.LAST_NAME_MIN_LENGTH })
        )
        .max(
          AUTH_CONSTANTS.LAST_NAME_MAX_LENGTH,
          t('validation.maxLength', { max: AUTH_CONSTANTS.LAST_NAME_MAX_LENGTH })
        ),
      email: z
        .string()
      ...
      password: z
        ...
        .min(
          ...
          t('validation.minLength', { min: AUTH_CONSTANTS.PASSWORD_MIN_LENGTH })
        )
        .max(
          AUTH_CONSTANTS.PASSWORD_MAX_LENGTH,
          t('validation.maxLength', { max: AUTH_CONSTANTS.PASSWORD_MAX_LENGTH })
        )
        .regex(
          /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/,
