...
export const TabList = styled.div<{ $variant: 'underline' | 'pill' }>`
  display: flex;
  align-items: center;
  gap: ${({ $variant }) => ($variant === 'pill' ? '12px' : '32px')};
  border-bottom: ${({ $variant, theme }) => 
    $variant === 'underline' ? `1px solid ${theme.colors.border.primary}` : 'none'};
  padding-bottom: ${({ $variant }) => ($variant === 'underline' ? '0' : '16px')};
  margin-bottom: 24px;
`;
...
export const TabButton = styled.button<{ $isActive: boolean; $variant: 'underline' | 'pill' }>`
  ...
  padding: ${({ $variant }) => ($variant === 'pill' ? '8px 16px' : '12px 0')};
  transition: all 0.3s;
  position: relative;
  ...
  ${({ $variant, $isActive, theme }) => 
    $variant === 'pill' && `
      border-radius: ${tkn('radius.sm')({ theme })};
